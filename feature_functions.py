...
class FeatureEngineering:
    ...
    def process_features(self, preprocessing_version, imbalance_strategy="weight"):
        """
        Process all feature types for a given preprocessing version AND save to disk.
        """
        print(f"\n{'='*80}")
        ...
        sparse.save_npz(os.path.join(output_dir, 'tfidf_X_train.npz'), tfidf_features['X_train'])
        sparse.save_npz(os.path.join(output_dir, 'tfidf_X_val.npz'), tfidf_features['X_val'])
        sparse.save_npz(os.path.join(output_dir, 'tfidf_X_test.npz'), tfidf_features['X_test'])
        ...
        # Save Word2Vec features (dense arrays)
        np.save(os.path.join(output_dir, 'word2vec_X_train.npy'), w2v_features['X_train'])
        np.save(os.path.join(output_dir, 'word2vec_X_val.npy'), w2v_features['X_val'])
        np.save(os.path.join(output_dir, 'word2vec_X_test.npy'), w2v_features['X_test'])
        print(f"Saved Word2Vec features to {output_dir}")
        ...
        # Save Mini Sentence-BERT features (dense arrays)
        np.save(os.path.join(output_dir, 'mini_sbert_X_train.npy'), msbert_features['X_train'])
        np.save(os.path.join(output_dir, 'mini_sbert_X_val.npy'), msbert_features['X_val'])
        np.save(os.path.join(output_dir, 'mini_sbert_X_test.npy'), msbert_features['X_test'])
        print(f"Saved Mini Sentence-BERT features to {output_dir}")
